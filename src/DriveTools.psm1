...
function Get-DriveScanPrediction {
<#
.SYNOPSIS
...
#>
    ...
    Set-DriveToolsStatus -Operation "Predict" -Details "Predicting scan duration for $resolvedPath"
    ...
    $enumerator = [System.IO.Directory]::EnumerateFiles($resolvedPath, "*", [System.IO.SearchOption]::AllDirectories).GetEnumerator()
    ...
    while ($true) {
        ...
        try {
            ...
            # Skip access errors or file-locked exceptions
        }
    }
    $stopwatch.Stop()
    ...
    if (-not $completedTraversal -and $filesCount -gt 0) {
        try {
            ...
            $driveQualifier = Split-Path $resolvedPath -Qualifier
            if ($driveQualifier) {
            ...
            # Ignore drive details error and use fallback scaling
            $estFileCount = $filesCount * 5
            $estTotalSize = $totalSize * 5
        }
    }
    ...
    if ($IncludeHashes) {
        ...
        $sampleCount = 0
        try {
            $enumerator2 = [System.IO.Directory]::EnumerateFiles($resolvedPath, "*", [System.IO.SearchOption]::AllDirectories).GetEnumerator()
            ...
            # Ignore search/access errors for samples
        }
        ...
        foreach ($f in $benchmarkFiles) {
            ...
            try {
                ...
                # Ignore hashing failures on individual files
            }
        }
        $hashStopwatch.Stop()
